...
<DIV class='aside_menu'>
  <ASIDE style='float: left;'><A href='../category4/list.jsp'>게시판</A>＞<A href='./list_category_grid1.jsp?categoryno=<%=categoryno %>&category_title=<%=category_title %>'><%=category_title %></A>> 수정 ＞ mp3 파일 신규등록</ASIDE> 
...
if (pds4Proc.passwordCheck(pdsno, passwd) == 1) { // 패스워드 일치
  ...
  if (sizeMP3 > 0) {
    if (sizeMP3 < 104857600) { // 1 ~ 104857600 byte, 100 MB
      ...
      pds4VO.setSize2(sizeMP3); // 저장된 mp3 파일 크기 VO 객체에 저장
      
      count = pds4Proc.create_mp3(pds4VO); // DBMS 적용
      if (count == 1) {
        %>
