...
<DIV class='aside_menu'>
  <ASIDE style='float: left;'><A href='../category4/list.jsp'>게시판</A>＞<A href='./list_category_grid1.jsp?categoryno=<%=categoryno %>&category_title=<%=category_title %>'><%=category_title %></A>> 수정 ＞Youtube 신규등록</ASIDE> 
  ...
  <DIV class='message'>
    ...
    if (pds4Proc.passwordCheck(pdsno, passwd) == 1) { // 패스워드 일치
      ...
      count = pds4Proc.create_youtube(pds4VO);
      if (count == 1) {
      %>
        <SPAN class='span_info'>Youtube를 등록했습니다.</SPAN>
      <%  
      } else {
      %>
        <SPAN class='span_warning'>Youtube 등록중에 에러가 발생하여 등록에 실패했습니다.</SPAN>
      <%  
      }
